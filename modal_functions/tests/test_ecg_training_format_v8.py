"""
v8: ECG digital com escala FIXA 2 px/mm.

  - Trace alpha gradient + boost
  - Resize com new_px_per_mm = 2.0 FIXO
  - Canvas adaptativo (>=900x450; expande se ECG nao couber)
  - Grid 232/245 em 2 px/mm
  - Centralizado
"""
...
logger = logging.getLogger("ecg_training_format_v8")
...
OUTPUT_DIR = RESULTS_ROOT / "ecg_training_format_v8"
...
def make_v8(cache):
    ...
    # Canvas: expande se ECG > usable area
...
def main() -> int:
    for stem in ECG_LIST:
        ...
        with cache_path.open("rb") as f:
            cache = pickle.load(f)
        img, new_w, new_h = make_v8(cache)
        out_path = OUTPUT_DIR / f"{stem}.png"
        cv2.imwrite(str(out_path), img)
        canvas_h, canvas_w = img.shape
        logger.info(
            "Salvo: %s (canvas %dx%d, ECG %dx%d a %.1f px/mm)",
