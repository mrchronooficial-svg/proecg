"""
v14: ~20% mais grosso que skeleton puro (ecg_digital).

Skeleton 1 px preto + halo cinza CLARO (200) — apenas uma "engordadinha"
sutil mantendo definicao da linha original.
"""
...
logger = logging.getLogger("ecg_training_format_v14")
...
OUTPUT_DIR = RESULTS_ROOT / "ecg_training_format_v14"
...
HALO_GRAY = 200  # halo claro -> ~20% engrosso perceptivo
...
def main() -> int:
    for stem in ECG_LIST:
        ...
        with cache_path.open("rb") as f:
            cache = pickle.load(f)
        img, new_w, new_h, n_skel, n_halo = make_v14(cache)
        out_path = OUTPUT_DIR / f"{stem}.png"
        cv2.imwrite(str(out_path), img)
        canvas_h, canvas_w = img.shape
        logger.info(
            "Salvo: %s (canvas %dx%d, ECG %dx%d a %.1f px/mm, nucleo %d px, halo %d px @ gray=%d)",
            out_path.name, canvas_w, canvas_h, new_w, new_h, FIXED_PX_PER_MM,
            n_skel, n_halo, HALO_GRAY,
        )
