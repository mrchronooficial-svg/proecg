"""
Roda pipeline ate Stenhede pra IMG_1575 e salva todos os outputs do Stenhede:
  - normalized (input do Stenhede, undistorted)
  - 4 canais do UNet (grid, text, signal, bg)
  - mascara B&W do canal 2 (tracado)
  - PixelSizeFinder visualizacao
"""
...
HEIC_PATH = Path(r"C:\Users\rafae\Downloads\IMG_1575.HEIC")
OUTPUT_DIR = Path(r"C:\Users\rafae\Desktop\Projeto ECG\IMG_1575_stenhede_steps")
...
def main() -> int:
    ...
    logger.info("Carregando %s", HEIC_PATH.name)
    img_bgr = heic_to_bgr(HEIC_PATH)
    h, w = img_bgr.shape[:2]
    if h > w * 1.2:
        img_bgr = cv2.rotate(img_bgr, cv2.ROTATE_90_COUNTERCLOCKWISE)

    # --- Pipeline pre-Stenhede ---
    ...
    # --- Salvar cada canal como heatmap individual ---
    channels = [
    ...
    # --- Mascara B&W do canal 2 (raw, threshold default) ---
    signal_binary = (signal_prob > SIGNAL_THRESHOLD).astype(np.uint8) * 255
    mask_bw = 255 - signal_binary  # tracado preto, fundo branco
    ...
    # --- 4 canais em grid combinado (similar a validacao_visual gen_08) ---
