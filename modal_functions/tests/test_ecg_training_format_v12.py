"""
v12: meio caminho entre v9 e v10 — anti-aliased (sem pontilhado).

Pipeline:
  1. Renderiza igual v8 (alpha + boost + resize + grid)
  2. Skeletonize (1 px) -> nucleo preto
  3. binary_dilation iterations=1 -> halo
  4. Nucleo fica preto (0), halo fica cinza (~128)
  5. Resultado: trace continuo de ~1.5 px perceptivos
"""
...
logger = logging.getLogger("ecg_training_format_v12")
...
OUTPUT_DIR = RESULTS_ROOT / "ecg_training_format_v12"
...
HALO_GRAY = 128  # nivel cinza do halo dilatado
...
def make_v12(cache):
    ...
    if not rows_with.any() or not cols_with.any():
        return np.full((MIN_CANVAS_H, MIN_CANVAS_W), 255, dtype=np.uint8), 0, 0, 0, 0
    ...
    # Nucleo (skel) + halo dilatado em cinza
    ...
    trace_layer[halo_only] = HALO_GRAY  # halo cinza
    trace_layer[skel_mask] = 0          # nucleo preto
    ...
    return canvas, new_w, new_h, int(skel_mask.sum()), int(halo_only.sum())
...
def main() -> int:
    for stem in ECG_LIST:
        ...
        with cache_path.open("rb") as f:
            cache = pickle.load(f)
        img, new_w, new_h, n_skel, n_halo = make_v12(cache)
        out_path = OUTPUT_DIR / f"{stem}.png"
        cv2.imwrite(str(out_path), img)
        canvas_h, canvas_w = img.shape
        logger.info(
            "Salvo: %s (canvas %dx%d, ECG %dx%d a %.1f px/mm, nucleo %d px, halo %d px)",
