    ...
    <section id="diagnostics" className="bg-white py-20 sm:py-32 lg:py-40">
      <div className="mx-auto max-w-5xl px-5 sm:px-6">
        <Reveal className="text-center">
          ...
          <h2 className="mt-3 text-[32px] font-semibold leading-[1.05] tracking-tight text-[#1d1d1f] sm:text-[48px] lg:text-[56px]">
        ...
        <div className="mt-12 divide-y divide-black/5 border-y border-black/5 sm:mt-16 lg:mt-20">
          {groups.map((g, i) => {
            ...
            return (
              <Reveal key={g.title} delay={i * 0.04}>
                ...
                >
                  <span className="text-[20px] font-semibold tracking-tight text-[#1d1d1f] sm:text-[24px] lg:text-[28px]">
                    {g.title}
                  </span>
                  <ChevronDown
                    className={`h-5 w-5 flex-shrink-0 text-[#1d1d1f]/55 transition-transform ${
                      open ? "rotate-180" : ""
                    }`}
                  />
                </button>
                ...
                >
                  <div className="overflow-hidden">
                    <div className="grid grid-cols-1 gap-x-8 gap-y-2 pb-6 text-[15px] text-[#1d1d1f]/75 sm:grid-cols-2 sm:pb-8 sm:text-[16px]">
                      {g.items.map((it) => (
                        <div key={it} className="flex items-center gap-2">
                          <span className="h-1 w-1 flex-shrink-0 rounded-full bg-[#0066CC]" />
                          <span>{it}</span>
                        </div>
                      ))}
                    </div>
