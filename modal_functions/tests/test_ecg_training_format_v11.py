"""
v11: meio caminho entre v9 (skeleton 1 px) e v10 (dilatado 2 px).

Dilatacao parcial — metade dos pixels novos (checkerboard, deterministico).

Pipeline:
  1. Renderiza igual v8 (alpha + boost + resize + grid)
  2. Skeletonize (1 px)
  3. binary_dilation iterations=1 -> 2 px
  4. Mantem apenas pixels novos que caem em padrao checkerboard
  5. Resultado: trace ~1.5 px em media
"""
...
logger = logging.getLogger("ecg_training_format_v11")
...
OUTPUT_DIR = RESULTS_ROOT / "ecg_training_format_v11"
...
def make_v11(cache):
    ...
    if not rows_with.any() or not cols_with.any():
        return np.full((MIN_CANVAS_H, MIN_CANVAS_W), 255, dtype=np.uint8), 0, 0, 0, 0.0, 0.0
    ...
    # Dilatation cheia (igual v10)
    skel_mask = trace_skel_img < DILATE_THRESHOLD
    dilated_full = binary_dilation(skel_mask, iterations=1)
    new_pixels = dilated_full & ~skel_mask  # so os adicionados

    # Checkerboard mask (deterministico): mantem so onde (y+x) par
    yy, xx = np.indices(new_pixels.shape)
    checker = ((yy + xx) % 2 == 0)
    kept_new = new_pixels & checker

    # Mascara final = skeleton original + metade dos novos
    half_dilated = skel_mask | kept_new

    trace_thick = np.full_like(trace_resized, 255)
    trace_thick[half_dilated] = 0
    ...
    dist = distance_transform_edt(half_dilated)
    if half_dilated.any():
        thickness_med = float(np.median(2 * dist[half_dilated]))
        thickness_max = float(np.max(2 * dist[half_dilated]))
    else:
        thickness_med = thickness_max = 0.0

    return canvas, new_w, new_h, int(half_dilated.sum()), thickness_med, thickness_max
...
def main() -> int:
    for stem in ECG_LIST:
        ...
        with cache_path.open("rb") as f:
            cache = pickle.load(f)
        img, new_w, new_h, n_dil, t_med, t_max = make_v11(cache)
        out_path = OUTPUT_DIR / f"{stem}.png"
        cv2.imwrite(str(out_path), img)
        canvas_h, canvas_w = img.shape
        logger.info(
            "Salvo: %s (canvas %dx%d, ECG %dx%d a %.1f px/mm, trace %d px, espessura med=%.1f max=%.1f)",
